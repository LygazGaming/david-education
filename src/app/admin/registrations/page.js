    ...
    <div className="min-h-screen flex bg-gray-100">
      ...
      <main className="flex-1 p-6">
        <div className="lg:hidden flex justify-between items-center mb-6">
          <h1 className="text-3xl font-semibold">Danh sách học viên</h1>
          <button
            ...
            <table className="min-w-full border">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="border px-4 py-2 text-left">Loại đăng ký</th>
                  <th className="border px-4 py-2 text-left">
                    Khóa học/Ca thử
                  </th>
              ...
              <tbody>
                {registrations.map((reg) => (
                  <tr key={reg._id} className="hover:bg-gray-50">
                    <td className="border px-4 py-2">{reg.studentName}</td>
                    <td className="border px-4 py-2">{reg.phone}</td>
                    <td className="border px-4 py-2">
                      {reg.type === "trial" ? "Tập thử" : "Học phần"}
                    </td>
                    <td className="border px-4 py-2">
                      {reg.type === "trial"
                        ? reg.trialSession
                        : reg.courseTitle || "Chưa xác định"}
                    </td>
                    <td className="border px-4 py-2">
